...
class ConnectsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create connect" do
    assert_difference('Connect.count') do
      post connects_url, params: { connect: { content: @connect.content, email: @connect.email, name: @connect.name } }
    end
  ...
  test "should update connect" do
    patch connect_url(@connect), params: { connect: { content: @connect.content, email: @connect.email, name: @connect.name } }
